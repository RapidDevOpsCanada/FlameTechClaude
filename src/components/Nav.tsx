    ...
    <nav className="sticky top-0 w-full z-50 bg-white/95 backdrop-blur-lg border-b border-line-light">
      <div className="max-w-7xl mx-auto px-6 md:px-10 flex items-center justify-between gap-6 h-[92px] md:h-[100px]">
        ...
        <div className="hidden md:flex items-center gap-4 shrink-0">
          ...
          <a
            href="#quote"
            className="inline-flex items-center rounded-full bg-emergency text-white font-extrabold uppercase tracking-tight px-6 py-3 text-[15px] hover:bg-white transition-colors"
          >
            Contact Us
          </a>
        </div>
...
function MegaPanel({ mega }: { mega: NonNullable<NavItem["mega"]> }) {
  ...
  return (
    <div className="absolute left-0 top-full pt-3 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
      <div
        className={`rounded-2xl bg-white border border-line-light soft-shadow overflow-hidden ${
                ...
                <ul className="space-y-1">
                  {group.items.map((item) => (
                    <li key={item.label}>
                      <Link
                        href={item.href}
                        className="flex items-start gap-3 p-3 rounded-xl hover:bg-emergency transition-colors group/item"
                      >
                        <Icon
                          name={item.icon}
                        ...
                        <span className="flex-1">
                          ...
                          {item.desc && (
                            <span className="block text-[13px] text-ink-500 mt-0.5 leading-snug">
                              {item.desc}
                            </span>
                          )}
                        </span>
          ...
          {mega.promo && (
            <div className="relative bg-white border-l border-line-light flex flex-col">
              <div className="relative h-36 overflow-hidden bg-cream-50 flex items-center justify-center">
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-emergency-deep via-emergency-deep/20 to-transparent" />
              </div>
              <div className="p-6 flex-1 flex flex-col">
                <h5 className="font-display text-xl font-extrabold tracking-tight text-ink-900 mb-2">
                  {mega.promo.title}
                </h5>
                <p className="text-sm text-ink-500 leading-relaxed mb-5 flex-grow">
                  {mega.promo.body}
                </p>
                <a
                  href={mega.promo.href}
                  className="inline-flex items-center justify-center gap-2 rounded-full bg-emergency text-white font-extrabold uppercase tracking-tight px-4 py-3 text-[13px] hover:bg-white transition-colors"
                >
                  {mega.promo.cta}
  ...
  if (!item.mega) {
    return (
      <Link
        href={item.href}
        onClick={onNavigate}
        className="block px-3 py-3 text-base font-semibold text-ink-900/90 rounded-xl hover:bg-white"
      >
        {item.label}
      </Link>
    );
  ...
  return (
    <div>
      <button
        ...
        className="w-full flex items-center justify-between px-3 py-3 text-base font-semibold text-ink-900/90 rounded-xl hover:bg-white"
      >
        {item.label}
        <Icon
          name="expand_more"
              ...
              <div className="space-y-1">
                {group.items.map((sub) => (
                  <Link
                    ...
                    href={sub.href}
                    onClick={onNavigate}
                    className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-white"
                  >
                    <Icon
                      name={sub.icon}
