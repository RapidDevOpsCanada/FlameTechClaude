...
- Lead form: posts directly to Formspree (no Next.js API route). Spam filtering + email delivery to the owner inbox are handled by Formspree's free tier. The production endpoint is hardcoded in `src/components/QuoteForm.tsx`; set `NEXT_PUBLIC_FORMSPREE_ENDPOINT` only if you want a staging deploy to use a different Formspree form.
...
- The lead form POSTs directly to Formspree (see `src/components/QuoteForm.tsx`). No `/api/lead` route, no Postgres writes, no Resend/Twilio wiring. Spam protection + email delivery to the owners' inbox are Formspree's job. The production endpoint is hardcoded in the form file; `NEXT_PUBLIC_FORMSPREE_ENDPOINT` overrides it when set (use this for staging or to point at a test form). Changing providers later means swapping the URL only — the form code is provider-agnostic.
- The `StickyCallBar` component intentionally returns `null` — the floating mobile call bar was implemented, tested, and pulled at the owner's request (the always-visible utility-bar phone link in the nav is the canonical mobile-call surface). Imports of it from pages are kept in place so re-enabling is a one-file change.
- Never ship `alt=""` on `<Image>` or `<img>`. SEO crawlers count empty alt as "missing alt" regardless of WCAG decorative-image rules, and our pre-commit hook will reject the commit. Every avatar / promo / decorative image in this repo derives meaningful alt text from an adjacent label (`alt={author}`, `alt={promo.title}`, etc.). If a future image is genuinely decorative, hide it from assistive tech with `aria-hidden` and a background-image instead — don't fall back to `alt=""`.
